...
# ---------------------------------------------------------------------------
# Chart 2 — Per-finger MJMPE (L / R subplots, both models)
# ---------------------------------------------------------------------------
...
def plot_finger_distribution(records, out_dir):
    ...
    for model in _MODELS:
        ...
        fig.suptitle(f"{model.capitalize()} — Per-Finger MJMPE Distribution (across sessions)")
        fig.tight_layout()
...
def plot_finger_heatmap(records, out_dir):
    col_labels = [f"L-{n[:3]}" for n in _FINGER_NAMES] + [f"R-{n[:3]}" for n in _FINGER_NAMES]
    ...
    for model in _MODELS:
        ...
        # Vertical separator between L and R halves
        ax.axvline(4.5, color="white", linewidth=2.5)

        plt.colorbar(im, ax=ax, label="MJMPE (px)")
        ax.set_title(f"{model.capitalize()} — Per-Finger MJMPE Heatmap (px)")
        fig.tight_layout()
